                                    ...
                                    <div className="grid gap-4">
                                      <div className="bg-gradient-to-r from-indigo-50 to-indigo-100 border-l-4 border-indigo-400 rounded-md px-4 py-2 dark:from-indigo-900 dark:to-slate-900 dark:border-indigo-700 mb-2 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between">
                                        <div>
                                          ...
                                          {(() => {
                                            const files = timeline.meeting?.attachments?.length
                                              ? timeline.meeting.attachments
                                              : timeline.meeting?.attachment
                                                ? [timeline.meeting.attachment]
                                                : [];
                                              ...
                                              <div className="mt-2 flex flex-wrap gap-2">
                                                {files.map((path, i) => (
                                                  <Link
                                                    ...
                                                    to={uploadUrl(path)}
                                                    target="_blank"
                                                    className="inline-flex items-center gap-1 rounded-full bg-indigo-600 text-white px-3 py-1 text-xs font-semibold dark:bg-indigo-400"
                                                  >
                                                    <Eye className="h-3 w-3" />
                                                    {files.length > 1 ? `Document ${i + 1}` : "View MOM document"}
                                                  </Link>
                                                ))}
                                              </div>
                                            );
